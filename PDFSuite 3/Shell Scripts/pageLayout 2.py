...
class Canvas(object):
	def __init__(self, pageSize, writeFilename):
		self.pageSize = pageSize
		self.writeFilename = writeFilename
		self.writeContext = Quartz.CGPDFContextCreateWithURL(CFURLCreateFromFileSystemRepresentation(kCFAllocatorDefault, writeFilename, len(self.writeFilename), False), self.pageSize, None)
		Quartz.CGContextBeginPage(self.writeContext, self.pageSize)
	
	def setStroke(color, alpha):
		red, green, blue = color[:]
		Quartz.CGContextSetRGBStrokeColor(self.writeContext, red, green, blue, alpha)
		
	def setFill(color, alpha):
		red, green, blue = color[:]
		Quartz.CGContextSetRGBFillColor(self.writeContext, red, green, blue, alpha)
